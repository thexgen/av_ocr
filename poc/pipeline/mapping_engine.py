...
from poc.pipeline.mapping_dictionary import (
    lookup_headers,
    save_learned_aliases,
)
from poc.pipeline.mapping_prompt import (
...
def _call_llm_for_unknown_headers(unknown_headers: list[str]) -> dict[str, Any]:
    ...
    if not llm_enabled or not QWEN_API_KEY:
        return {
            "field_mapping": {},
            "confidence": 40,
            "warnings": [
    ...
    logger.info(
        "Calling Qwen for %s unknown header(s) only: %s",
        len(unknown_headers),
        unknown_headers,
    )
    ...
    response = client.chat.completions.create(
        ...
        messages=[
            {"role": "system", "content": UNKNOWN_HEADERS_SYSTEM_PROMPT},
            {"role": "user", "content": build_unknown_headers_prompt(unknown_headers)},
        ],
    )

    content = response.choices[0].message.content or ""
    logger.info("Raw Qwen response preview: %s", content[:500])
    cleaned = _strip_json_fence(content)
    try:
        parsed = json.loads(cleaned)
    except json.JSONDecodeError as exc:
        logger.error("Failed to parse Qwen JSON: %s", exc)
